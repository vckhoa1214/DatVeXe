                                ...
                                <div class="table-responsive">
                                    <table class="table table-hover col-12">
                                        ...
                                        @foreach ($nhaxes as $nhaxe)
                                            <tr>
                                                <td class="hihi">{{ $nhaxe->id }}</td>
                                                <td class="hihi">{{ $nhaxe->name }}</td>
                                                <td class="hihi">{{ $nhaxe->phoneNo[0] ?? '' }}</td>
                                                <td class="hihi">{{ $nhaxe->address[0] ?? '' }}</td>
                                                {{-- <td class="hihi">{{ $nhaxe->description }}</td>
                                                <td class="hihi">{{ $nhaxe->policy }}</td> --}}
                                                ...
                                                <td class="d-flex pt-3 px-0">
                                                    <div class="d-flex align-items-center">
                                                        ...
                                                        <button
                                                            ...
                                                            onclick="window.location.href='{{ route('dashboard.suanhaxe', ['id' => $nhaxe->id]) }}'">
                                                        </button>

                                                        <!-- Form xóa -->
                                                        <form action="{{ route('dashboard.xoanhaxe') }}" method="POST"
                                                              onsubmit="return submitForm(this);" class="m-0">
                                                            @csrf
                                                            <input type="hidden" name="id" value="{{ $nhaxe->id }}">
                                                            <button type="submit"
...
<script>
    function navigateTo(id) {
        window.location.href = `/dashboard/quanlynhaxe/chitietnhaxe/${id}`;
    }

    function submitForm(form) {
        swal({
            title: "Bạn có chắc chắn muốn xóa nhà xe này không ?",
            text: "Bạn sẽ không thể khôi phục lại nhà xe và các chuyến xe lẫn vé của khách hàng liên quan !",
            icon: "warning",
            buttons: true,
            dangerMode: true,
        })
    ...
    var content = document.getElementsByClassName("hihi");
    for (var i = 0; i < content.length; i++) {
        var contentText = content[i].innerText;
        if (contentText.length > 80) {
            content[i].innerText = contentText.substring(0, 80) + "...";
        }
    }
</script>
